...
export default function StatsChart({ commutes, moods, myUuid }: StatsChartProps) {
  ...
  return (
    <div className="bg-background border border-gray-700 rounded-md p-4">
      <h2 className="text-foreground font-bold mb-4 text-lg">📊 오늘의 통계</h2>
      {(myGoRank || myLeaveRank) && (
        <div className="mb-3 text-center text-sm">
          <span className="font-bold text-primary">{myGoRank ? `출근 ${myGoRank}등` : ''}</span>
          {myGoRank && myLeaveRank && <span className="mx-2">|</span>}
          <span className="font-bold text-primary">{myLeaveRank ? `퇴근 ${myLeaveRank}등` : ''}</span>
          <span className="ml-2 text-muted-foreground text-xs">(출근 {goRecords.length}명, 퇴근 {leaveRecords.length}명)</span>
        </div>
      )}
      <div className="grid grid-cols-3 gap-2 mb-4">
        ...
        <div className="text-center">
          <div className="text-base font-bold text-foreground mb-1">🌅</div>
          <div className="text-muted-foreground text-xs">출근왕</div>
          <div className="text-foreground text-xs mt-1 truncate">
            {earlyBird}
          </div>
        </div>
        <div className="text-center">
          <div className="text-base font-bold text-foreground mb-1">🌙</div>
          <div className="text-muted-foreground text-xs">칼퇴왕</div>
          <div className="text-foreground text-xs mt-1 truncate">
            {nightOwl}
          </div>
        </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 justify-center items-stretch mb-2">
        {/* 기분 분포 (BarChart) */}
        <div className="flex-1 min-w-[180px] bg-background border border-gray-700 rounded-md flex flex-col items-center p-2">
          <h3 className="text-foreground text-xs font-medium mb-2 text-center flex items-center">
          ...
          {moodDistribution.length > 0 ? (
            ...
            <div className="text-muted-foreground text-center py-4 text-xs">데이터 없음</div>
          )}
        </div>
        {/* 출근 시간 분포 (BarChart) */}
        <div className="flex-1 min-w-[180px] bg-background border border-gray-700 rounded-md flex flex-col items-center p-2">
          <h3 className="text-foreground text-xs font-medium mb-2 text-center">🌅 출근 시간 분포</h3>
          <Bar
            data={{
        ...
        {/* 퇴근 시간 분포 (BarChart) */}
        <div className="flex-1 min-w-[180px] bg-background border border-gray-700 rounded-md flex flex-col items-center p-2">
          <h3 className="text-foreground text-xs font-medium mb-2 text-center">🌙 퇴근 시간 분포</h3>
          <Bar
            data={{
