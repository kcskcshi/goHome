...
interface SupabaseContextType {
  ...
  addGameScore: (score: number, nickname: string, game?: string) => Promise<void>;
...
export const SupabaseProvider = ({ children }: { children: ReactNode }) => {
  ...
  // 게임 스코어 추가 (게임 타입별)
  const addGameScore = async (score: number, nickname: string, game: string = 'commantle') => {
    try {
      console.log('[addGameScore] params', { nickname, game, score });
      ...
      if (exist) {
        ...
        const { error } = await getSupabase()
          .from('game_scores')
          .insert([{ nickname, game, score, created_at: createdAt }])
          .select();
        if (error) {
